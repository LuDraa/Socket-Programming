...
class FileTransferService(file_transfer_pb2_grpc.FileTransferServiceServicer):
    ...
    def TransferFile(self, request, context):
        ...
        if len(self.throughputs) >= 2:
            ...
            print(f"Average Throughput after {len(self.throughputs)} files: {average_throughput} bytes per second")
            print(f"Standard Deviation of Throughput after {len(self.throughputs)} files: {std_dev_throughput} bytes per second")
        elif len(self.throughputs) == 1:
            average_throughput = self.throughputs[0]
            print(f"Only one file transferred. Average Throughput: {average_throughput} bytes per second")
            print("Standard Deviation of Throughput: 0 bytes per second")
        else:
            print("No files transferred yet. Average Throughput and Standard Deviation not available.")
